/datum/martial_art/fencing
	name = "Fencing"
	id = MARTIALART_FENCING
...
/datum/martial_art/fencing/teach(mob/living/owner, make_temporary=FALSE)
	if(..())
...
/datum/martial_art/fencing/on_remove(mob/living/owner)
...
/datum/martial_art/fencing/proc/check_streak(mob/living/attacker, mob/living/defender)
	switch(streak)
		if("neck_chop")
			streak = ""
...
/datum/martial_art/fencing/proc/leg_sweep(mob/living/attacker, mob/living/defender)
...
/datum/martial_art/fencing/proc/quick_choke(mob/living/attacker, mob/living/defender)//is actually lung punch
...
/datum/martial_art/fencing/proc/neck_chop(mob/living/attacker, mob/living/defender)
...
/datum/martial_art/fencing/grab_act(mob/living/attacker, mob/living/defender)
	if(check_streak(attacker, defender))
		return TRUE
	log_combat(attacker, defender, "grabbed (Fencing)")
	..()

/datum/martial_art/fencing/harm_act(mob/living/attacker, mob/living/defender)
	if(check_streak(attacker, defender))
		return TRUE
...
/datum/martial_art/fencing/disarm_act(mob/living/attacker, mob/living/defender)
	if(check_streak(attacker, defender))
		return TRUE
	...
	log_combat(attacker, defender, "shoved (Fencing)", "[stuff_in_hand ? " removing \the [stuff_in_hand]" : ""]")
	return FALSE
...
/obj/item/clothing/gloves/fencing/equipped(mob/user, slot)
	. = ..()
	if(slot & ITEM_SLOT_GLOVES)
		style.teach(user, TRUE)

/obj/item/clothing/gloves/fencing/dropped(mob/user)
	. = ..()
	if(user.get_item_by_slot(ITEM_SLOT_GLOVES) == src)
		style.remove(user)

/obj/item/clothing/gloves/fencing/sec//more obviously named, given to sec
	name = "fencing gloves"
	desc = "These gloves can teach you to perform Fencing using nanochips."
...
/obj/item/clothing/gloves/fencing/combatglovesplus
	name = "combat gloves plus"
	desc = "These tactical gloves are fireproof and electrically insulated, and through the use of nanochip technology will teach you the martial art of fencing."
	icon_state = "black"
	...
	max_heat_protection_temperature = GLOVES_MAX_TEMP_PROTECT
	resistance_flags = NONE
	armor_type = /datum/armor/fencing_combatglovesplus

/datum/armor/fencing_combatglovesplus
	bio = 90
	fire = 80
	acid = 50
