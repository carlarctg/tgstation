...
/datum/mutation/human/superweapon/proc/metamorph()
	...
	spew_gas(cocoon_turf) // Most alterations by the cocoon will spew a small amount of a random gas, to make things even trippier. Shouldn't cause too much trouble and will be funny.

	// Run this proc for each tile next to the victim, with the proc being delayed by 0.2 seconds multiplied by the amount of nearby open tiles.
	var/counter = 0
	for(var/turf/open/cocoonable_ground in orange(1, cocoon_turf))
		addtimer(CALLBACK(src, PROC_REF(bleoeurghf), cocoonable_ground), 0.2 SECONDS * counter) // timer runs fast
		counter++
...
/datum/mutation/human/superweapon/proc/finish_morpheus()
	...
	for(var/obj/structure/alien/resin/wall/superweapon_cocoon/cocoon_wall in cocoon_bits)
		addtimer(CALLBACK(src, PROC_REF(shwip), cocoon_wall), rand(0.2 SECONDS, 1 SECONDS))
	...
	if(metamorph_type.current_stage < metamorph_type.total_stages)
		// The cycle continues.
		warning_timer = addtimer(CALLBACK(src, PROC_REF(warning), METAMORPH_COCOON_TIME - 2 MINUTES))
		metamorph_timer = addtimer(CALLBACK(src, PROC_REF(metamorph), METAMORPH_COCOON_TIME))
...
/datum/mutation/human/superweapon/proc/shwip(obj/structure/alien/resin/wall/superweapon_cocoon/cocoon_wall)
	...
	spew_gas(get_turf(cocoon_wall))
	qdel(cocoon_wall)

/// Spews a randomly-heated randomly-small amount of any gas in the game. Fun!
/datum/mutation/human/superweapon/proc/spew_gas(atom/source)
	...
	source_turf.atmos_spawn_air("[picked_gas]=[rand(5, 20)];TEMP=[(T20C + rand(-10, 10))]")
...
#define ADDED_MUTATIONS 1
#define ADDED_QUIRKS 2
#define ADDED_TRAUMAS 3
...
/datum/metamorph_type/proc/on_chrysallis()
	...
	for(var/datum/mutation/human/mut as anything in mutations_to_add)
		mut.stabilizer_coeff = 0 // no instability
		mut.power_coeff = 1.5 // guaranteed power
		mut.synchronizer_coeff = 0.5 // less self-effects
		mut.energy_coeff = 0.5 // faster use
		// why the last 3? superweapons cant use almost anything but their mutations and they even have bad back
		// so making their mutations slightly better will hopefully help them actually use their powerset rather than resort
		// to average tider self-defence things
		victim.dna.add_mutation(mut, MUT_OTHER)
...
/datum/metamorph_type/cryonics/handle_chrysallis()
	...
	switch(current_stage)
		...
		if(3)
			// Stage 3, get over your SAD and become festively jolly, become a frickin snow golem! Also thermal vision.
			...
			// Snow golems get FREE cryokinesis already. I mean they could get double cryokinesis i guess??
			//victim.remove_mutation(/datum/mutation/human/cryokinesis)
			victim.set_species(/datum/species/golem/snow)
